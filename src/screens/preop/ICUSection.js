...
// ─────────────────────────────────────────────────────────────────────────────
// ICUSection
// Folds all four ICU calculators (APACHE II, SOFA, RASS, CAM-ICU) into a
// collapsible section for use inside the Preoperative screen.
//
// Props
//   patient  – { weight, age, height, gender }  (from PreoperativeScreen state)
// ─────────────────────────────────────────────────────────────────────────────

export default function ICUSection({ patient = {} }) {
  // ── Accordion: only one card open at a time ──────────────────────────────
  const [activeCard, setActiveCard] = useState(null);
  const toggleCard = (cardKey, nextOpen) => setActiveCard(nextOpen ? cardKey : null);
  ...
  const handleCalculateApache = () => {
    ...
    const aps =
      tScore + mScore + hrScore + rrScore + oxyScore +
      phScore + naScore + kScore + creatScore + hctScore + wbcScore + gcsScore;
  ...
  const handleCalculateSofa = () => {
    ...
    let interpretation;
    let type;
    if (total <= 6) { interpretation = 'Low mortality risk (<10%)'; type = 'success'; }
    else if (total <= 9) { interpretation = 'Moderate mortality risk (15-20%)'; type = 'warning'; }
    else if (total <= 12) { interpretation = 'High mortality risk (40-50%)'; type = 'warning'; }
    else if (total <= 14) { interpretation = 'Very high mortality risk (~50%)'; type = 'danger'; }
    else { interpretation = 'Very high mortality risk (>80%)'; type = 'danger'; }
  ...
  return (
    <View>
      {/* Section header – matches the HTML template's "ICU Calculators" heading */}
      <View style={styles.sectionHeader}>
        <Text style={styles.sectionHeaderText}>ICU Calculators</Text>
        <Text style={styles.sectionHeaderSub}>Critical care assessment tools and organ failure scoring</Text>
      </View>
      ...
      >
        ...
        <TextInput
          ...
          keyboardType="decimal-pad"
          placeholder="4.0"
          value={apachePotassium}
          onChangeText={setApachePotassium}
        />
      ...
      >
        <PickerSelect
          ...
          options={[
            { value: '0', label: '≥400 (0 points)' },
            { value: '1', label: '300-399 (1 point)' },
            { value: '2', label: '200-299 (2 points)' },
            { value: '3', label: '100-199 with ventilation (3 points)' },
            { value: '4', label: '<100 with ventilation (4 points)' },
          ]}
          selected={sofa.respiratory}
          onSelect={(v) => setSofa((p) => ({ ...p, respiratory: v }))}
        />
        <PickerSelect
          label="Coagulation (Platelets ×10³/μL)"
          options={[
            { value: '0', label: '≥150 (0 points)' },
            { value: '1', label: '100-149 (1 point)' },
            { value: '2', label: '50-99 (2 points)' },
            { value: '3', label: '20-49 (3 points)' },
            { value: '4', label: '<20 (4 points)' },
          ]}
          selected={sofa.coagulation}
          onSelect={(v) => setSofa((p) => ({ ...p, coagulation: v }))}
        />
        <PickerSelect
          label="Liver (Bilirubin mg/dL)"
          options={[
            { value: '0', label: '<1.2 (0 points)' },
            { value: '1', label: '1.2-1.9 (1 point)' },
            { value: '2', label: '2.0-5.9 (2 points)' },
            { value: '3', label: '6.0-11.9 (3 points)' },
            { value: '4', label: '≥12.0 (4 points)' },
          ]}
          selected={sofa.liver}
          onSelect={(v) => setSofa((p) => ({ ...p, liver: v }))}
        />
        <PickerSelect
          label="Cardiovascular"
          options={[
            { value: '0', label: 'No hypotension (0 points)' },
            { value: '1', label: 'MAP <70 mmHg (1 point)' },
            { value: '2', label: 'Dopamine ≤5 or dobutamine (2 points)' },
            { value: '3', label: 'Dopamine >5 or epi ≤0.1 or norepi ≤0.1 (3 points)' },
            { value: '4', label: 'Dopamine >15 or epi >0.1 or norepi >0.1 (4 points)' },
          ]}
          selected={sofa.cardiovascular}
          onSelect={(v) => setSofa((p) => ({ ...p, cardiovascular: v }))}
        />
        <PickerSelect
          label="Central Nervous System (Glasgow Coma Scale)"
          options={[
            { value: '0', label: '15 (0 points)' },
            { value: '1', label: '13-14 (1 point)' },
            { value: '2', label: '10-12 (2 points)' },
            { value: '3', label: '6-9 (3 points)' },
            { value: '4', label: '3-5 (4 points)' },
          ]}
          selected={sofa.cns}
          onSelect={(v) => setSofa((p) => ({ ...p, cns: v }))}
        />
        <PickerSelect
          ...
          options={[
            { value: '0', label: '<1.2 (0 points)' },
            { value: '1', label: '1.2-1.9 (1 point)' },
            { value: '2', label: '2.0-3.4 (2 points)' },
            { value: '3', label: '3.5-4.9 or UO <500mL/day (3 points)' },
            { value: '4', label: '≥5.0 or UO <200mL/day (4 points)' },
          ]}
          selected={sofa.renal}
          onSelect={(v) => setSofa((p) => ({ ...p, renal: v }))}
        />
      ...
      >
        <PickerSelect
          label="Patient's Level of Consciousness"
          options={[
      ...
      >
        <PickerSelect
          ...
          selected={cam.acute}
          onSelect={(v) => setCam((p) => ({ ...p, acute: v }))}
        />
        <PickerSelect
          label="Feature 2: Inattention"
          options={[
            { value: 'no', label: 'No (0-2 errors on attention screening)' },
            { value: 'yes', label: 'Yes (>2 errors on attention screening)' },
          ]}
          selected={cam.inattention}
          onSelect={(v) => setCam((p) => ({ ...p, inattention: v }))}
        />
        <PickerSelect
          label="Feature 3: Disorganized thinking"
          options={[
            { value: 'no', label: 'No (0-1 errors on thinking assessment)' },
            { value: 'yes', label: 'Yes (>1 error on thinking assessment)' },
          ]}
          selected={cam.disorganized}
          onSelect={(v) => setCam((p) => ({ ...p, disorganized: v }))}
        />
        <PickerSelect
          label="Feature 4: Altered level of consciousness"
          options={[
            { value: 'no', label: 'No (RASS = 0)' },
            { value: 'yes', label: 'Yes (RASS ≠ 0)' },
          ]}
          selected={cam.altered}
          onSelect={(v) => setCam((p) => ({ ...p, altered: v }))}
        />
...
const styles = StyleSheet.create({
  // Section header – mirrors the HTML template's collapsible category heading
  sectionHeader: {
  ...
  // Shared input / label styles – mirrors ICUCalculatorsScreen and PreoperativeScreen
  label: {
    fontSize: 14,
    fontWeight: '600',
    color: COLORS.text,
